...
func InitQueries() {
	...
	SELECT_USER_BY_EMAIL_QUERY = fmt.Sprintf("SELECT id, email, password, scopes, name FROM %s WHERE email = ? AND is_deleted = false", userTableName)
	CHECK_USER_EXISTS_BY_EMAIL_QUERY = fmt.Sprintf("SELECT count(1) FROM %s WHERE email = ? AND is_deleted = false", userTableName)
	...
	UPDATE_PASSWORD_QUERY = fmt.Sprintf("UPDATE %s SET password=?, forgot_password_token='', last_password_changed_at = ? WHERE id=? OR email=?", userTableName)
